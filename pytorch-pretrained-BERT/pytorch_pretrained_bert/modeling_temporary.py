...
class MultimodalBertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, visual,acoustic,token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
...
class ETSBertModel(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config,newly_added_config):
        super(ETSBertModel, self).__init__(config)
        self.newly_added_config = newly_added_config
        self.embeddings = BertEmbeddings(config)
        self.map_audio_visual = Audio_visual_mapper(config,newly_added_config)
        self.encoder = BertEncoder(config)
...
class MultimodalBertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, newly_added_config,num_labels):
        super(MultimodalBertForSequenceClassification, self).__init__(config)
...
class ETSBertForSequenceClassification(BertPreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    """
    def __init__(self, config, newly_added_config,num_labels):
        super(ETSBertForSequenceClassification, self).__init__(config)
        self.config = config
        self.newly_added_config = newly_added_config
...
class IncongruityBertForSequenceClassification(BertPreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    def __init__(self, config, num_labels):
        super(IncongruityBertForSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.config = config
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.storyteller = nn.LSTMCell(config.hidden_size, config.story_size)####Need to add story size
        self.sigmoid = nn.Sigmoid()
        self.classifier = nn.Linear(config.story_size, num_labels)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, has_context = None):
        ...
        if has_context:
            ...
            h_t,c_t = h_init,c_init
            for i in xrange(5):
                h_t, c_t = self.storyteller(pooled_output[:,i,:],(h_t,c_t))
            context_story = h_t
            punchline_story = self.classifier(pooled_output[:,5,:]) ###Pass it through nonlinear network
            distance = torch.norm(context_story - punchline_story,p=2,dim=1)
            prob = self.sigmoid(distance)
